...
const Dashboard: React.FC = () => {
  const { phases, stats, updateProgress } = useDashboard();

  const statItems = [
    { label: 'Days Active', value: stats.daysActive.toString(), icon: Calendar, color: 'text-blue-600' },
    { label: 'Goals Completed', value: stats.goalsCompleted.toString(), icon: Target, color: 'text-green-600' },
    { label: 'Growth Score', value: stats.growthScore, icon: TrendingUp, color: 'text-purple-600' },
    { label: 'Achievements', value: stats.achievements.toString(), icon: Award, color: 'text-orange-600' },
  ];
